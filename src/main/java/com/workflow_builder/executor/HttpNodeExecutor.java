...
public class HttpNodeExecutor implements NodeExecutor {
    ...
    public CompletableFuture<Map<String, Object>> execute(NodeDefinition node, Map<String, Object> input,
        ...
        // timeout from config
        int timeoutSeconds = Optional.ofNullable(cfg.get("timeoutSeconds")).map(Object::toString).map(Integer::valueOf)
                .orElse(30);
        ...
        return respMono.timeout(Duration.ofSeconds(timeoutSeconds))
                .map(resp -> {
                    ...
                    try {
                        ...
                        if (resp != null && (resp.trim().startsWith("{") || resp.trim().startsWith("["))) {
                            ObjectMapper mapper = new ObjectMapper();
                            Object json = mapper.readValue(resp, Object.class);
                            summary.put("output", json);
                        } else {
